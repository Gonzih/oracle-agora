...
class ArcClient {
    ...
    async rpc(method, params = []) {
        ...
        const response = await axios_1.default.post(this.rpcUrl, request, {
    ...
    async getBlockNumber() {
        const hex = await this.rpc("eth_blockNumber");
        return parseInt(hex, 16);
    }
    async getChainId() {
        const hex = await this.rpc("eth_chainId");
        return parseInt(hex, 16);
    }
    ...
    async logTradeIntent(params) {
        if (!this.connected) {
            console.log(`[arc:offline] Would log trade intent: ${params.position} ${params.usdcAmount} USDC on ${params.marketId}`);
            return;
        }
        const blockNumber = await this.getBlockNumber();
        console.log(`[arc] Trade intent logged at block ${blockNumber}: ${params.position} $${params.usdcAmount} USDC on market ${params.marketId}`);
    }
}
