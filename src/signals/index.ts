...
import { PolyScoutClient } from "./polyScoutClient.js";
import type { AggregatedSignal, ConsensusSignal } from "./types.js";
 ...
 */
const DEMO_SIGNALS: AggregatedSignal[] = [
  {
    marketId: "0x1234demo",
...
export class SignalAggregator {
  private scout: PolyScoutClient;
  private scoutAvailable = false;

  constructor() {
    this.scout = new PolyScoutClient();
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.warn("[signals] poly-scout unavailable, using demo signals:", (err as Error).message);
  ...
  async fetchSignals(): Promise<AggregatedSignal[]> {
    if (!this.scoutAvailable) {
    ...
    try {
      ...
      console.error("[signals] Live fetch failed, falling back to demo:", (err as Error).message);
      return DEMO_SIGNALS.map((s) => ({ ...s, timestamp: new Date().toISOString() }));
    }
  }

  private async fetchLiveSignals(): Promise<AggregatedSignal[]> {
    ...
    const enriched = await Promise.allSettled(
      topConsensus.map((c) => this.enrichWithLateMoney(c))
    );
  ...
  private async enrichWithLateMoney(consensus: ConsensusSignal): Promise<AggregatedSignal> {
    let lateMoney;
    try {
  ...
  private scoreSignal(
    ...
    lateMoney?: { confidenceScore: number; volumeSpikeMultiplier: number }
  ): number {
